...
function check(name, score, max, detail, fix) {
  return { name, score: Math.max(0, Math.min(max, Math.round(score * 100) / 100)), max, detail, fix };
}
...
async function coreArchitecture() {
  ...
  // 1a. Package structure (3pts)
  {
    ...
    if (pkg) {
      ...
      const hasTurbo = fileExists(path.join(ROOT, 'turbo.json')) || parsed.devDependencies?.turbo || parsed.dependencies?.turbo;
      const hasPnpm = fileExists(path.join(ROOT, 'pnpm-workspace.yaml')) || parsed.workspaces;
      if (hasTurbo || hasPnpm) { score++; parts.push('workspace config'); }
      const scripts = parsed.scripts || {};
      if (scripts.build || scripts.dev) { score++; parts.push('build/dev scripts'); }
    }
    checks.push(check('Package structure', score, 3,
      parts.length > 0 ? parts.join(', ') : 'No package.json found',
      score < 3 ? 'Ensure package.json has turbo/pnpm workspace config and build/dev scripts' : null));
  }
  ...
  // 1b. Core wrapper (3pts)
  {
    ...
    checks.push(check('Core wrapper', score, 3,
  ...
  // 1c. POS SDK (3pts)
  {
    ...
    checks.push(check('POS SDK', score, 3,
  ...
  // 1d. Server health (3pts)
  {
    ...
    checks.push(check('Server health', score, 3, detail,
...
async function agentSystem() {
  ...
  // 2a. SOUL.md exists (2pts)
  {
    ...
    checks.push(check('SOUL.md exists', score, 2,
      lines > 100 ? `aros-ai/SOUL.md: ${lines} lines` : lines > 0 ? `SOUL.md too short (${lines} lines, need >100)` : 'aros-ai/SOUL.md not found',
  ...
  // 2b. Agent source (2pts)
  {
    ...
    checks.push(check('Agent source', exists ? 2 : 0, 2,
  ...
  // 2c. Shre control (2pts)
  {
    ...
    checks.push(check('Shre control', score, 2,
      hasDir ? `aros-ai/shre-control/ has ${fileCount} file(s)` : 'aros-ai/shre-control/ not found',
  ...
  // 2d. 6 core agents defined (3pts)
  {
    ...
    checks.push(check('6 core agents defined', score, 3,
  ...
  // 2e. Trust gate wired (3pts)
  {
    ...
    checks.push(check('Trust gate wired', score, 3, detail,
...
async function skillsEcosystem() {
  ...
  // 3a. Skills directory (2pts)
  {
    ...
    checks.push(check('Skills directory', exists ? 2 : 0, 2,
  ...
  // 3b. Skills manifest (3pts)
  {
    ...
    if (!content) {
      checks.push(check('Skills manifest', 0, 3, 'skills/AROS-SKILLS.md not found',
      ...
      checks.push(check('Skills manifest', score, 3,
        `${categories} skill categories in AROS-SKILLS.md`,
        score < 3 ? `Add more skill categories (have ${categories}, need 8 for full score)` : null));
    }
  }
  ...
  // 3c. Skill implementations (3pts)
  {
    ...
    checks.push(check('Skill implementations', score, 3,
      `${fileCount} .ts/.mjs files in skills/src/`,
      score < 3 ? `Add more skill implementations (have ${fileCount}, need 20 for full score)` : null));
  }
  ...
  // 3d. Skill documentation (2pts)
  {
    ...
    const content = readFile(manifestPath);
    if (!content) {
      checks.push(check('Skill documentation', 0, 2, 'No AROS-SKILLS.md to check',
      ...
      checks.push(check('Skill documentation', score, 2,
...
async function dataConnectors() {
  ...
  // 4a. Connector directory (2pts)
  {
    ...
    checks.push(check('Connector directory', exists ? 2 : 0, 2,
  ...
  {
    const rapidrms = fileExists(path.join(connDir, 'rapidrms-api.ts')) ||
                     fileExists(path.join(connDir, 'rapidrms.ts')) ||
                     dirExists(path.join(connDir, 'rapidrms'));
    checks.push(check('RapidRMS connector', rapidrms ? 2 : 0, 2,
  ...
  {
    const verifone = fileExists(path.join(connDir, 'verifone.ts')) ||
                     fileExists(path.join(connDir, 'verifone-api.ts')) ||
                     dirExists(path.join(connDir, 'verifone'));
    checks.push(check('Verifone connector', verifone ? 2 : 0, 2,
  ...
  {
    const storepulse = fileExists(path.join(connDir, 'storepulse-link.ts')) ||
                       fileExists(path.join(connDir, 'storepulse.ts')) ||
                       fileExists(path.join(connDir, 'storepulse-bridge.ts'));
    checks.push(check('StorePulse link', storepulse ? 2 : 0, 2,
...
async function marketplaceNodes() {
  ...
  // 5a. Marketplace directory (2pts)
  {
    ...
    checks.push(check('Marketplace directory', exists ? 2 : 0, 2,
  ...
  // 5b. Registry (3pts)
  {
    ...
    checks.push(check('Registry', score, 3,
      found ? `marketplace/${found} found (${score === 3 ? 'comprehensive' : 'basic'})` : 'No registry file found',
  ...
  // 5c. Node types defined (3pts)
  {
    ...
    const registryContent = readFile(path.join(mktDir, 'registry.ts')) +
    ...
    const typeMatches = (registryContent.match(/type\s*[:=]|nodeType|node_type|NodeType/gi) || []).length;
    const nodeCount = Math.max(nodeFiles, typeMatches);
    const score = nodeCount >= 8 ? 3 : nodeCount >= 5 ? 2 : nodeCount >= 2 ? 1 : 0;
    checks.push(check('Node types defined', score, 3,
  ...
  // 5d. Plugin dev docs (2pts)
  {
    ...
    checks.push(check('Plugin dev docs', score, 2,
      exists ? `docs/plugin-development/ has ${fileCount} file(s)` : 'docs/plugin-development/ not found',
...
async function licensingSecurity() {
  ...
  // 6a. License system (3pts)
  {
    ...
    if (!checkDir) {
      checks.push(check('License system', 0, 3, 'No licensing/ directory found',
      ...
      const hasBootGuard = files.some(f => /boot.?guard/i.test(f)) || allContent.includes('bootGuard') || allContent.includes('boot-guard');
      const hasValidator = files.some(f => /validat/i.test(f)) || allContent.includes('validate');
      const hasKeygen = files.some(f => /keygen/i.test(f)) || allContent.includes('generateKey') || allContent.includes('keygen');
      ...
      checks.push(check('License system', score, 3,
        `${parts.join(', ')} found in licensing/`,
        score < 3 ? `Add missing: ${[!hasBootGuard && 'boot-guard', !hasValidator && 'validator', !hasKeygen && 'keygen'].filter(Boolean).join(', ')}` : null));
    }
  }

  // 6b. ECDSA keys (2pts)
  {
    const licDir = dirExists(path.join(ROOT, 'licensing')) ? path.join(ROOT, 'licensing') :
                   dirExists(path.join(ROOT, 'src', 'licensing')) ? path.join(ROOT, 'src', 'licensing') : null;
    let found = false;
    if (licDir) {
    ...
    checks.push(check('ECDSA keys', found ? 2 : 0, 2,
      found ? 'P-256/ECDSA references found in licensing code' : 'No ECDSA/P-256 references found',
  ...
  // 6c. Encryption (2pts)
  {
    ...
    for (const dir of searchDirs) {
      ...
      const files = listDir(dir).filter(f => f.endsWith('.ts') || f.endsWith('.mjs') || f.endsWith('.js'));
      for (const f of files) {
        const content = readFile(path.join(dir, f));
        if (/AES-256-GCM|aes-256-gcm|createCipheriv|createDecipheriv|encrypt.*decrypt/i.test(content)) {
          found = true;
          break;
        }
      }
    ...
    checks.push(check('Encryption', found ? 2 : 0, 2,
  ...
  // 6d. Security docs (3pts)
  {
    ...
    const hasReadme = exists && (fileExists(path.join(secDir, 'README.md')) || fileExists(path.join(secDir, 'SECURITY.md')));
    const score = !exists ? 0 : fileCount >= 3 && hasReadme ? 3 : fileCount >= 2 ? 2 : 1;
    checks.push(check('Security docs', score, 3,
      exists ? `security/ has ${fileCount} file(s)${hasReadme ? ' with docs' : ''}` : 'security/ not found',
...
async function billingSignup() {
  ...
  // 7a. Billing module (2pts)
  {
    ...
    checks.push(check('Billing module', exists ? 2 : 0, 2,
  ...
  // 7b. Stripe integration (2pts)
  {
    ...
    checks.push(check('Stripe integration', found ? 2 : 0, 2,
  ...
  // 7c. Signup flow (2pts)
  {
    ...
    checks.push(check('Signup flow', found ? 2 : 0, 2,
  ...
  // 7d. Pricing tiers (2pts)
  {
    ...
    checks.push(check('Pricing tiers', score, 2,
...
async function deploymentConfig() {
  ...
  {
    const dockerCompose = fileExists(path.join(ROOT, 'deploy', 'docker-compose.yml')) ||
                          fileExists(path.join(ROOT, 'docker-compose.yml'));
    checks.push(check('Docker config', dockerCompose ? 2 : 0, 2,
  ...
  // 8b. K8s manifests (2pts)
  {
    ...
    const score = exists && fileCount > 0 ? 2 : 0;
    checks.push(check('K8s manifests', score, 2,
  ...
  // 8c. Whitelabel system (2pts)
  {
    ...
    const score = exists && fileCount > 0 ? 2 : 0;
    checks.push(check('Whitelabel system', score, 2,
  ...
  // 8d. AROS config (2pts)
  {
    ...
    checks.push(check('AROS config', configFile ? 2 : 0, 2,
...
async function documentation() {
  ...
  // 9a. Platform docs (2pts)
  {
    ...
    checks.push(check('Platform docs', score, 2,
      lines > 0 ? `docs/PLATFORM.md: ${lines} lines${lines <= 500 ? ' (need >500)' : ''}` : 'docs/PLATFORM.md not found',
  ...
  // 9b. Industry taxonomy (2pts)
  {
    ...
    checks.push(check('Industry taxonomy', exists ? 2 : 0, 2,
  ...
  // 9c. Agent docs (2pts)
  {
    ...
    checks.push(check('Agent docs', exists ? 2 : 0, 2,
  ...
  // 9d. Onboarding docs (2pts)
  {
    ...
    checks.push(check('Onboarding docs', exists ? 2 : 0, 2,
      exists ? `${customerOnboard ? 'CUSTOMER' : 'DEVELOPER'}-ONBOARDING.md exists` : 'No onboarding docs found',
  ...
  // 9e. Architecture doc (2pts)
  {
    ...
    checks.push(check('Architecture doc', exists ? 2 : 0, 2,
...
async function integrationUplink() {
  ...
  // 10a. Uplink architecture (3pts)
  {
    ...
    checks.push(check('Uplink architecture', score, 3,
      exists ? `AROS-UPLINK-ARCHITECTURE.md: ${lines} lines` : 'AROS-UPLINK-ARCHITECTURE.md not found',
  ...
  // 10b. Developer portal (3pts)
  {
    ...
    if (exists) {
      ...
      if (dirExists(path.join(portalDir, 'portal'))) { score++; parts.push('portal/ subdir'); }
      if (dirExists(path.join(portalDir, 'sdks'))) { score++; parts.push('sdks/ subdir'); }
    }
    checks.push(check('Developer portal', score, 3,
  ...
  // 10c. SDK packages (2pts)
  {
    ...
    const langs = dirExists(sdksDir) ? listDir(sdksDir).filter(f => {
      const p = path.join(sdksDir, f);
      try { return fs.statSync(p).isDirectory(); } catch { return false; }
    }) : [];
    const score = langs.length >= 4 ? 2 : langs.length >= 2 ? 1 : 0;
    checks.push(check('SDK packages', score, 2,
      langs.length > 0 ? `${langs.length} SDK language(s): ${langs.join(', ')}` : 'No SDKs found in aros-developer-portal/sdks/',
  ...
  // 10d. Web UI (2pts)
  {
    ...
    checks.push(check('Web UI', score, 2,
      hasPkg ? 'apps/web/ with package.json' : exists ? 'apps/web/ exists but missing package.json' : 'apps/web/ not found',
...
function printReport(categories, total, maxTotal) {
  ...
  console.log(`\u2551${pad('', (W - 40) / 2)}${C.bold}${C.cyan}AROS Platform — Benchmark${C.reset}${pad('', (W - 40) / 2 + 14)}  \u2551`);
  ...
  console.log(`\u2551  ${pad('TOTAL SCORE', 28)}${totalBar}  ${pad(`${total}/${maxTotal}`, 7)}\u2551`);
  ...
  for (const cat of categories) {
    ...
    for (const ch of cat.checks) {
      const icon = ch.score === ch.max ? `${C.green}\u2713${C.reset}` :
                   ch.score > 0 ? `${C.yellow}\u25CB${C.reset}` :
                   `${C.red}\u2717${C.reset}`;
      console.log(line(`  ${icon} ${pad(ch.name, 24)} ${ch.score}/${ch.max}  ${C.dim}${ch.detail}${C.reset}`));
      if (fixMode && ch.fix) {
...
async function main() {
  ...
  const total = categories.reduce((s, cat) =>
    s + cat.checks.reduce((cs, ch) => cs + ch.score, 0), 0);
  const maxTotal = categories.reduce((s, cat) => s + cat.max, 0);
  const grade = gradeFor(total);

  // Build JSON output
  const catKeys = [
    'core_architecture', 'agent_system', 'skills_ecosystem',
    'data_connectors', 'marketplace_nodes', 'licensing_security',
    'billing_signup', 'deployment_config', 'documentation',
    'integration_uplink',
  ];
  const jsonResult = {
